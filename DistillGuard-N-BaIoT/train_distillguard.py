...
CHECKPOINT_DIR = "/Users/nadeemyousaf/.gemini/antigravity/scratch/DistillGuard-N-BaIoT/checkpoints"
...
def load_data():
    dataset_path = os.path.join(DATA_DIR, 'n_baiot_datasets.pt')
    if not os.path.exists(dataset_path):
        raise FileNotFoundError("Run data_processor.py first.")
    
    datasets = torch.load(dataset_path, weights_only=False)
    
    # Dataloaders - Increased Batch Size for N-BaIoT
    # Teacher (32 -> 128), Student (64 -> 256)
    t_loader = DataLoader(datasets['train'], batch_size=128, shuffle=True)
    t_val = DataLoader(datasets['val'], batch_size=128, shuffle=False)
    
    s_loader = DataLoader(datasets['train'], batch_size=256, shuffle=True)
    s_val = DataLoader(datasets['val'], batch_size=256, shuffle=False)
    
    return t_loader, t_val, s_loader, s_val, datasets
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(CHECKPOINT_DIR, "teacher_best.pth")):
        ...
        teacher = train_teacher(teacher, t_loader, t_val, epochs=5) # Reduced to 5 for N-BaIoT speed

    # 3. Fine-tune Teacher (Adversarial) - Step 7
    # Use epsilon=0.1 as per paper range 0.01-0.1
    teacher = fine_tune_teacher_adversarial(teacher, t_loader, epochs=2, epsilon=0.05) # Reduced to 2
    ...
    student = train_student_sgkd(student, teacher, s_loader, s_val, epochs=5) # Reduced to 5
